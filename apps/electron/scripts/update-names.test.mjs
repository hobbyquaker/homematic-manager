...
import {checkBuilderConfig, checkOutput, expandName} from './update-names.mjs';
...
describe('a packaging output', () => {
    ...
    it('passes when every manifest entry is a file with a safe name', () => {
        ...
        expect(result).toEqual({manifests: ['latest-linux.yml', 'latest.yml'], problems: []});
    });
    ...
    it('fails on a manifest that names nothing', () => {
        const result = checkOutput(output({'latest-mac.yml': 'version: 1.0.0\n'}), yaml);
