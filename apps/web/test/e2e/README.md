...
| `smokeGroups.spec.ts` | smoke groups in the device list (task 58) with a device set of its own: a BidCos team row with its detectors under it, the detectors' team column and the members dialog (read-only - hm-simulator 1.0 has no `setTeam`); an HmIP smoke group created from the toolbar with one `GROUP_1` write per detector in the simulator's write log, its row and column, a member taken out with one `GROUP_1: false` write |
