...
| the `controls` of `profiles/*.json` and `master-metadata.json`, `easymode-time-selectors.json`, `extracted/easymode_controls.json.gz` | the WebUI's easymode Tcl and its localisation (`/www/config/easymodes`, `/www/webui/js/lang`), read by `scripts/easymode-controls.mjs` | HMSL 2.0                                      |
