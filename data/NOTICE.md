...
| the `controls` of `profiles/*.json`, `easymode-time-selectors.json`, `extracted/easymode_controls.json.gz`      | the WebUI's easymode Tcl and its localisation (`/www/config/easymodes`), read by `scripts/easymode-controls.mjs`   | HMSL 2.0                                      |
