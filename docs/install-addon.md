...
| An interface shows as "waiting" | In the first two minutes after the app started, an interface whose port refuses or does not answer is tried again after 1, 2, 4 and 8 seconds and then every 15 seconds, because its process may simply not be up yet - on a system that is still booting, or on openccu-lite where the app starts before the interface processes. It connects as soon as the process answers. One that still does not answer after two minutes shows as "not present" or "not answering". |
